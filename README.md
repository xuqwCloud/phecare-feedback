...
PheCare 是一款面向 PKU 宝宝家长与照护者的 iPhone 与 iPad 日常记录工具。它帮助用户整理每日食物、特殊配方、血 Phe 检验结果及成长数据，并在设备本地完成换算和趋势展示。
...
> **重要说明**
>
> PheCare 是家庭日常记录、数值换算与信息整理工具。App 会展示注明来源的年龄参考范围、可修改的初始记录值以及相对用户目标的数值对照，但不会据此生成个体化行动建议。所有范围、初值、换算和趋势仅用于记录和信息参考，不应用于自行调整饮食方案或特殊配方用量。
...
食物品种、产地、加工状态、品牌配方和生产批次都可能影响实际营养值。App 中的估算值仅用于辅助记录，用户应以产品包装标签、厂家资料及自己确认的数据来源为准。

年龄膳食 Phe 范围和血 Phe 记录参考范围来自《苯丙氨酸羟化酶缺乏症饮食治疗与营养管理共识》，《中华儿科杂志》2019 年第 57 卷第 6 期 405–409 页，DOI: `10.3760/cma.j.issn.0578-1310.2019.06.002`。App 内“参考依据与计算说明”页面会进一步列出当前规则版本、初始值规则、单位换算和 Phe/Tyr 推导方法。
...
- 可以上传不包含敏感信息的界面截图
- **不要上传宝宝姓名、出生日期、血值、检测机构信息、完整饮食记录或导出的备份文件**
- 本仓库仅处理产品使用与数据问题，不针对具体儿童提供个体化判断或用量建议

## Bug 反馈建议包含

1. iPhone 或 iPad 型号和系统版本
2. PheCare 版本号
3. 问题发生前的操作步骤
4. 预期结果与实际结果
5. 已脱敏的截图或错误提示

如果问题涉及计算，请使用虚构或脱敏示例说明，不要公开真实的儿童个人记录。
...
- 真实个人信息或未成年人隐私数据
- 针对具体儿童的个体状态判断、方案制定或配方用量咨询
- 重复刷屏、广告或无关推广
- 盗版、破解及其他违法违规内容

## 联系方式

技术支持、隐私问题和产品反馈可发送邮件至 [xbeichenbei@gmail.com](mailto:xbeichenbei@gmail.com)，也可以通过 [GitHub Issues](../../issues) 联系。请勿在邮件或公开 Issue 中发送宝宝身份信息、真实血值或完整备份文件。
